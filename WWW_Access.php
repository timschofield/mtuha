...
if (!isset($SelectedRole)) {
	$FormName = 'Access1';
	echo '<form name="Access1" onSubmit="return VerifyForm(this);" method="post" action="' . htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8') . '" method="post" class="noPrint standard">';
	...
	InputText($FormName, 'SecRoleName', _('Role Title'), _('A name for this security role.'), 40, 40, True, array(), $_POST['SecRoleName']);
	SubmitButton( _('Create Role'), 'Submit', 'submitbutton');
	echo '</form>';
	...
	while ($myrow = DB_fetch_array($result)) {
		...
		printf('<td>%s</td>
				<td><a class="editlink" href="%s&amp;SelectedRole=%s">' . _('Edit') . '</a></td>
				<td><a class="deletelink" href="%s&amp;SelectedRole=%s&amp;delete=1&amp;SecRoleName=%s" onclick="return MakeConfirm(\'' . _('Are you sure you wish to delete this role?') . '\', \'Confirm Delete\', this);">' . _('Delete') . '</a></td>
...
if (isset($SelectedRole)) {
	...
	while ($AvailRow = DB_fetch_array($Result)) {
		...
		if (in_array($AvailRow['tokenid'], $TokensUsed)) {
			printf('<td>%s</td>
					<td>%s</td>
					<td><a href="%sSelectedRole=%s&amp;remove=1&amp;PageToken=%s" onclick="return MakeConfirm(\'' . _('Are you sure you wish to remove this security token from this role?') . '\', \'Confirm Delete\', this);">' . _('Remove') . '</a></td>
					<td>&nbsp;</td>
					<td>&nbsp;</td>
